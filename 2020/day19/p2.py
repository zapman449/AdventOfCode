...
def build_regex(rules: typing.Dict[str, typing.Dict], idx: str) -> str:
    ...
    if rule['type'] == 'letter':
        ...
        for sub_list in rule['value']:
            looper = False
            looper_idx = 0
            for i, sub_idx in enumerate(sub_list):
                if sub_idx == idx:
                    looper = True
                    looper_idx = i
                    continue
                sub_regex.append(build_regex(rules, sub_idx))
        if first:
            first = False
            regex = "(" + sub_regex
        else:
...
def main() -> None:
    ...
    rules["8"] = {'type': 'list_or_list', 'value': [["42", ], ["42", "8"]]}
    rules["11"] = {'type': 'list_or_list', 'value': [["42", "31"], ["42", "11", "31"]]}
    # pprint.pprint(rules)

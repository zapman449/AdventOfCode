...
class Item(object):
    ...
    def inspect(self, operation: str, test_product: int) -> int:
        ...
        if v2 == "old":
            ...
            v2i = int(v2)
        if instruction == "+":
            self._worry_level = (v1i + v2i) % test_product
        elif instruction == "*":
            self._worry_level = (v1i * v2i) % test_product
        else:
...
class Monkey(object):
    ...
    def throws_to(self, test_product: int) -> typing.Tuple[typing.Union[Item, None], int]:
        if len(self.items) == 0:
            return None, -1
        i = self.items.pop(0)
        i.inspect(self.operation, test_product)
...
def main() -> None:
    ...
    for line in fileinput.input():
        ...
        if sline.startswith("Monkey"):
            ...
            test_product = test_product * int(words[3])
    ...
    for round_num in range(1, 10001):
        for monkey in monkeys:
            while True:
                i, target = monkey.throws_to(test_product)
                if i is None:
                    break
                monkeys[target].catch(i)
        if round_num in (1, 20, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000):
            for idx, m in enumerate(monkeys):
                print(f"after round {round_num} Monkey {idx} inspected items {int(m)} times")
            print()
